    ...
    async def shutdown(self):
        if self.producer is not None:
            try:
                ...
                self.producer = None
                print(f"[KafkaCallback] Producer for topic '{self.topic}' shut down.")
            except Exception as e:
                print(f"[WARN] KafkaCallback.shutdown() failed: {e}")
...
class ClickHouseLiquidationsDevKafka(KafkaCallback):
